...
class GUI(Ui_MainWindow):
    ...
    def __init__(self, window):
        ...
        self.num_cores.setValidator(QtGui.QIntValidator(1,99))
    ...
    def simulation(self):
        ...
        try:
            ...
            # --------------- seleção de redes ---------------
            n_fibers = 1
            n_cores = int(self.num_cores.text())
            n_modes = 1
            ...
            if self.WDMButton.isChecked():
                ...
                # --------------- se o botão de crosstalk está selecionado ou não ---------------

                if self.no_crosstalk.isChecked():
                    consider_xt = 0
                elif self.yes_crosstalk.isChecked():
                    consider_xt = 1
                else:
                ...
                if self.Sim_ruidoASE.isChecked():
                    damp = float(self.dist_amplificador_lineEdit.text())
                    consider_ase_noise = 1

                if self.Nao_ruidoASE.isChecked():
                    damp = 0.0
                    consider_ase_noise = 0
            ...
            if self.chamadas_radioButton.isChecked():
                ...
                for load in range(min_traffic_load, max_traffic_load, traffic_load_step):
                    ...
                    damp, n_cores, mcf, consider_xt), callback=load_bp.append)
                pool.close()
                pool.join()
                t2 = time.time()
            ...
            # --------------- se o usuário preferir a simulação com o numero de bloqueios ---------------
                ...
                t1 = time.time()
                for load in range(min_traffic_load, max_traffic_load, traffic_load_step):
                    r = pool.apply_async(simulation.FixedBlockages, args=(
                ...
                for percentage in range(min_traffic_load, max_traffic_load, traffic_load_step):
                    ...
                    consider_ase_noise, damp, n_cores, mcf, consider_xt), callback=load_bp.append)
                pool.close()
                pool.join()
                t2 = time.time()
                ...
                try:
                    ...
                    damp, n_calls, traffic_load, str(load_bp))
                    cursor.execute(comando_SQL, dados)
                    banco.commit()
                    print(cursor.rowcount, "salvo no banco de dados!")
