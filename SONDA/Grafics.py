...
class Grafics:
    ...
    @staticmethod
    def plot_crosstalk_distance(load_bp):

        dist, xt = zip(*load_bp)

        plt.plot(dist, xt, linewidth=4)

        plt.xscale('log')
        plt.xlabel('Distância (m)')
        plt.ylabel('$\it{Intercore}$ $\it{Crosstalk}$ (dB)')
        # plt.title("Crosstalk X Distance - SONDA")
        plt.grid(True)
        plt.show()
...
'''
g = Grafics()        
g.plot_blocking_probability([(1, 0.8452), (7, 0.4929), (12, 0.3397), (19, 0.2113)])
'''